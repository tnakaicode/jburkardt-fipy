...
def p_laplacian(my_grid=16, my_degree=1, my_p=1.5):
    ...
    filename = 'p_laplacian_mesh_{:03d}_{:02d}_{:.02f}.png'.format(
        my_grid, my_degree, my_p)
    ...
    #
    uexpr = Expression("x[0] * ( 1.0 - x[0] ) * x[1] * ( 1.0 - x[1] )",
                       degree=10)
    ...
    plt.colorbar(fig)
    filename = 'p_laplacian_solution_{:03d}_{:02d}_{:.02f}.png'.format(
        my_grid, my_degree, my_p)
    plt.savefig(filename)
    print('  Graphics saved as "%s"' % (filename))
    plt.close()
    ...
    plt.colorbar(fig)
    filename = 'p_laplacian_gradun_{:03d}_{:02d}_{:.02f}.png'.format(
        my_grid, my_degree, my_p)
    plt.savefig(filename)
    print('  Graphics saved as "%s"' % (filename))
    plt.close()
    ...
    plt.colorbar(fig)
    filename = 'p_laplacian_gammau_{:03d}_{:02d}_{:.02f}.png'.format(
        my_grid, my_degree, my_p)
    plt.savefig(filename)
    print('  Graphics saved as "%s"' % (filename))
    plt.close()
