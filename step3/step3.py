...
def step3 ( my_grid, my_degree ):

#*****************************************************************************80
#
## step3 solves a Poisson equation with piecewise constant diffusivity K(X,Y).
#
#  Discussion:
#
...
#
#    For this case, I want to add the computation and plotting of |grad u|:
#      |grad u| = sqrt ( ududx^2 + dudy^2 )
#
#  Modified:
#
#    25 October 2018
...
#  Plot the mesh.
#
  plot ( my_mesh, title = 'step3 Mesh' )
  filename = 'step3_mesh.png'
...
#
  ...
  plot ( diffusivity ( x ), title = 'step3 Diffusivity' )
  filename = 'step3_diffusivity.png'
  plt.savefig ( filename )
  print ( '  Graphics saved as "%s"' % ( filename ) )
  plt.close ( )
#
...
#  Plot the solution u.
#
  fig = plot ( u, title = 'step3 solution' )
  plt.colorbar ( fig )
  filename = 'step3_solution.png'
  plt.savefig ( filename )
  print ( '  Graphics saved as "%s"' % ( filename ) )
  plt.close ( )
#
...
#
  ...
  fig = plot ( gradun, title = 'step3 |grad u|' )
  plt.colorbar ( fig )
  filename = 'step3_gradun.png'
  plt.savefig ( filename )
  print ( '  Graphics saved as "%s"' % ( filename ) )
  plt.close ( )
#
...
#
  fig = plot ( e, title = 'step3 indicators' )
  plt.colorbar ( fig )
  filename = 'step3_indicators.png'
  plt.savefig ( filename )
  print ( '  Graphics saved as "%s"' % ( filename ) )
  plt.close ( )
#
...
#
  ...
  print ( '  Step 3 in P-Laplacian investigation.' )
