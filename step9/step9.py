...
#
    if (my_case == 1):
        ...
        f_expr = Expression(
            "-2*exp(pow(x[0],2)+pow(x[1],2))*(sin(x[0]+x[1])*(1+2*(pow(x[0],2)+pow(x[1],2))) + 2*cos(x[0]+x[1])*(x[0]+x[1]))", degree=10)
    elif (my_case == 3):
        u_exact_expr = Expression(
            "2*(1+x[1])/(pow(3+x[0],2)+pow(1+x[1],2))", degree=10)
        f_expr = Expression("0", degree=0)
    elif (my_case == 4):
        u_exact_expr = Expression("exp(-pow(x[0],2)-pow(x[1],2))", degree=10)
        f_expr = Expression(
            "-(-4*pow(x[0],2)-4*pow(x[1],2)+4)*exp(-pow(x[0],2)-pow(x[1],2))", degree=10)
    else:
        raise Exception(
            '  The value of my_case should be between 1 and 4, your value was: {}'.format(my_case))
#
